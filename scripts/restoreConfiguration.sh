...
docker run --name=""$RESTORE_CONTAINER"" --rm --volumes-from ${CONTAINER_NAME} -v $(pwd)/${backup_file}:/backup.tar busybox tar xfv /backup.tar jenkins/jobs jenkins/plugins jenkins/secrets jenkins/users jenkins/logs jenkins/secret.* jenkins/*.xml
